...
function renderSequenceList(){
 ...
 list.innerHTML='';
 sequenceItems.forEach((item,index)=>{const li=document.createElement('li');const selected=item.variants.find(v=>v.media_id===sequenceSelections[item.shot_id])||item.variants[item.variants.length-1];li.className='sequence-item '+(index===sequenceIndex?'active ':'')+(selected?'available':'missing');let options=item.variants.map(v=>'<option value="'+v.media_id+'" '+(selected&&v.media_id===selected.media_id?'selected':'')+'>'+v.version+' · '+v.review_status+'</option>').join('');const inputLink=item.input_path?'<a class="sequence-input-link" href="'+item.input_path+'" target="_blank" rel="noopener">↗ הקלט שנשלח למודל</a>':'';li.innerHTML='<div class="sequence-row"><span class="sequence-shot-id">'+item.shot_id+'</span><div><strong>'+item.title+'</strong><small>'+(selected?'גרסה נבחרת: '+selected.version:'חסר — טרם נוצר')+'</small>'+inputLink+'</div><select aria-label="בחירת גרסה עבור '+item.shot_id+'" '+(selected?'':'disabled')+'>'+options+'</select></div>';const select=li.querySelector('select');if(select)select.onchange=()=>{sequenceSelections[item.shot_id]=select.value;localStorage.setItem('sequenceSelections',JSON.stringify(sequenceSelections));if(index===sequenceIndex)setSequenceSource();else renderSequenceList()};const link=li.querySelector('.sequence-input-link');if(link)link.onclick=e=>e.stopPropagation();li.querySelector('.sequence-row').onclick=e=>{if(e.target===select||e.target===link)return;if(selected){sequenceIndex=index;setSequenceSource()}else{document.querySelector('#sequenceMessage').textContent=item.shot_id+' עדיין חסר; אי אפשר להוסיף אותו לרצף.';renderSequenceList()}};list.appendChild(li)});
}
...
async function loadSequence(){
 try{const response=await fetch('../data/media_registry_v0.1.json',{cache:'no-store'});if(!response.ok)throw new Error('registry');const registry=await response.json();const titles={S001:'הטקס',S002:'שוקו א׳ מול המראה',S003:'שוקו ה׳ לא עונה',S004:'המסלול מתחיל',S005:'הדלת הראשונה',S006:'החבר מצטרף',S007:'שוקו ג׳ בבית הקפה',S008:'שוקו ד׳ והמפתח'};const inputVersions={S001:'v0.1',S002:'v0.2',S003:'v0.4',S004:'v0.2',S005:'v0.4',S006:'v0.2',S007:'v0.2',S008:'v0.3',S009:'v0.4',S010:'v0.4',S011:'v0.4',S012:'v0.4',S013:'v0.4',S014:'v0.4',S015:'v0.5',S016:'v0.5'};sequenceItems=sequenceOrder.map(shot=>{const variants=registry.media.filter(item=>item.shot_id===shot&&item.kind==='video'&&item.review_url);const inputPath=inputVersions[shot]?`../data/${shot}_video_result_${inputVersions[shot]}.json`:null;return{shot_id:shot,title:titles[shot]||shot,variants,input_path:inputPath}});renderSequenceList();setSequenceSource()}catch{document.querySelector('#sequenceMessage').textContent='לא ניתן לטעון את Media Registry כרגע.'}}
