...
const portalModelSelect=document.querySelector('#portalModelSelect'),portalJobForm=document.querySelector('#portalJobForm'),portalJobMessage=document.querySelector('#portalJobMessage');
let portalModels=[];
function portalJobInput(){const form=new FormData(portalJobForm),chosen=portalModels.find(m=>m.key===form.get('model'));return{shot_id:String(form.get('shot_id')).toUpperCase(),version:'portal-proposal',provider:chosen?.provider,model:chosen?.model,duration_seconds:Number(form.get('duration_seconds')),prompt:String(form.get('prompt')),references:[]}}
async function portalApi(path,options={}){const response=await fetch(path,{headers:{'content-type':'application/json'},...options});const result=await response.json();if(!response.ok)throw new Error(result.message||result.error||'Portal API unavailable');return result}
async function loadPortalModels(){if(!portalModelSelect)return;try{const data=await portalApi('/api/models');portalModels=data.providers.flatMap(provider=>provider.models.map(model=>({...model,provider:provider.provider,key:provider.provider+'|'+model.model})));portalModelSelect.innerHTML=portalModels.map(model=>`<option value="${model.key}">${model.provider} · ${model.display_name}</option>`).join('');portalJobMessage.textContent='בחר פעולה, בדוק עלות, ואז בקש אישור מפורש.'}catch{portalModelSelect.innerHTML='<option>הפורטל הציבורי הוא מצב צפייה; פתח את הפורטל המקומי כדי ליצור jobs.</option>';portalModelSelect.disabled=true}}
document.querySelector('#estimatePortalJob')?.addEventListener('click',async()=>{try{const estimate=await portalApi('/api/estimate',{method:'POST',body:JSON.stringify(portalJobInput())});portalJobMessage.textContent=`${estimate.provider} · ${estimate.model} · ${estimate.duration_seconds}s · $${estimate.estimated_cost_usd}. ${estimate.warnings||''}`}catch(error){portalJobMessage.textContent=error.message}});
portalJobForm?.addEventListener('submit',async event=>{event.preventDefault();try{const input=portalJobInput(),estimate=await portalApi('/api/estimate',{method:'POST',body:JSON.stringify(input)});const ok=window.confirm(`לאשר יצירת job?\n${estimate.provider}\n${estimate.model}\n${estimate.duration_seconds} שניות\nעלות משוערת: $${estimate.estimated_cost_usd}`);if(!ok){portalJobMessage.textContent='לא נוצר job ולא בוצעה פעולה בתשלום.';return}const job=await portalApi('/api/jobs',{method:'POST',body:JSON.stringify(input)});const approved=await portalApi(`/api/jobs/${job.id}/approve`,{method:'POST',body:JSON.stringify({approved:true,note:'Approved in portal UI'})});portalJobMessage.textContent=`${approved.id} אושר ונשמר. ${approved.submission.reason}`}catch(error){portalJobMessage.textContent=error.message}});
loadPortalModels();
