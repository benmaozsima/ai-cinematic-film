...
export function DirectorChat({ film, isHebrew, onChanged, onOpenAdvanced }: Props) {
  ...
  const runNeedsReview = latestRun?.status === 'needs_attention';
  const runActive = !!latestRun && ['queued', 'generating', 'awaiting_media', 'awaiting_qc', 'paused'].includes(latestRun.status);
  ...
  return <section className="director-chat" aria-label={he ? 'צ׳ט הבמאי' : 'Director chat'} aria-busy={busy}>
    ...
    <div className="director-chat-compose">
      ...
      {runNeedsReview ? <div className="director-chat-actions director-active-action" role="status">
        <div><Check /><span><b>{he ? 'הטייקים מוכנים לבדיקה' : 'Generated takes are ready for review'}</b><small>{latestRun?.nextAction || (he ? 'בדוק ובחר את הטייקים כדי לחבר את הסרט.' : 'Review and select the takes to assemble the film.')}</small></span></div>
        <Button onClick={onOpenAdvanced}>{he ? 'בדיקת הטייקים והמשך' : 'Review takes and continue'}</Button>
      </div> : runActive ? <div className="director-chat-actions director-active-action" role="status">
