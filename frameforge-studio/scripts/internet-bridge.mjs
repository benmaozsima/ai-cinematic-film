...
const page = `<!doctype html><html lang="he" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Frameforge — כניסה לפורטל</title><style>body{background:#141711;color:#edf0e6;font:18px system-ui;display:grid;place-items:center;min-height:95vh;margin:0}form{width:min(360px,85vw);display:grid;gap:18px;padding:32px;border:1px solid #434b39;border-radius:14px}h1{margin:0}input,button{font:inherit;padding:12px;border-radius:6px;border:1px solid #707960}input{width:100%;box-sizing:border-box}button{background:#c5e18a;cursor:pointer}p{color:#b8c2ac}</style><form method="post" action="/bridge-login"><h1>FRAMEFORGE</h1><p>כניסה לפורטל יצירת הסרטים</p><label>שם משתמש<input name="username" autocomplete="username" required dir="ltr"></label><label>סיסמה<input name="password" type="password" autocomplete="current-password" required dir="ltr"></label><button>כניסה לפורטל הראשי</button></form></html>`;
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/bridge-login') {
    ...
    if (!equal(values.get('username') || '', username) || !equal(values.get('password') || '', password)) {
      ...
      return res.end(page.replace('כניסה לפורטל יצירת הסרטים', 'שם המשתמש או הסיסמה שגויים. נסו שוב.'));
    }
    const token = randomBytes(32).toString('hex');
    sessions.set(token, Date.now() + 86400000);
    res.writeHead(303, { Location: '/', 'Set-Cookie': `frameforge_session=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=86400` });
    return res.end();
  }
  ...
  const headers = { ...req.headers, host: 'localhost:3210' };
  delete headers.cookie;
  delete headers.authorization;
  delete headers['x-forwarded-host'];
  if (headers.origin) headers.origin = 'http://localhost:3210';
  const upstream = request({ hostname: '127.0.0.1', port: 3210, path: req.url, method: req.method, headers }, (response) => {
